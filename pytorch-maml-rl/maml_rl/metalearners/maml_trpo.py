...
class MAMLTRPO(GradientBasedMetaLearner):
    ...
    async def adapt(self, train_futures, first_order=None, args=None, inner=None):
        ...
        for futures in train_futures:
            inner_loss = reinforce_loss(self.policy,
                                        ...
                                        params=params)
            if inner:
                params = self.policy.update_params_inner(inner_loss,
                                                         params=params,
                                                         step_size=self.fast_lr,
                                                         first_order=first_order,
                                                         args=args)
            else:
                params = self.policy.update_params_outer(inner_loss,
                                                         params=params,
                                                         step_size=self.fast_lr,
                                                         first_order=first_order)
        return params
    ...
    async def surrogate_loss(self, train_futures, valid_futures, old_pi=None, args=None, inner=None):
    ...
    def step(self,
        ...
        old_losses, old_kls, old_pis = self._async_gather([
            self.surrogate_loss(train, valid, old_pi=None, args=args, inner=True)
        ...
        for _ in range(ls_max_steps):
            ...
            losses, kls, _ = self._async_gather([
                self.surrogate_loss(train, valid, old_pi=old_pi, args=args, inner=False)
